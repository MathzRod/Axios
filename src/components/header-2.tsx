'use client';
// 👉 Diz ao Next.js que este componente será renderizado no cliente (necessário por usar states e efeitos).

import React from 'react';
// 👉 Importa React para usar useState, useEffect, etc.
import { Button, buttonVariants } from '@/components/ui/button';
// 👉 Importa seu componente Button e as variações de estilo buttonVariants.
import { cn } from '@/lib/utils';
// 👉 Função 'cn' serve para juntar classes do Tailwind de forma condicional.
import { MenuToggleIcon } from '@/components/menu-toggle-icon';
// 👉 Ícone animado (menu hamburguer → X).
import { useScroll } from '@/components/use-scroll';
// 👉 Hook customizado que detecta se a página foi rolada X pixels.
...
export function Header() {
	...
	// Estado 'open': controla se o menu mobile está aberto (true) ou fechado (false).

	const scrolled = useScroll(10);
	// Variável 'scrolled': fica true quando o usuário rola mais de 10px.

	const links = [
	{ label: "About me", href: "#about-me", onClick: () => scrollToId("about-me") },
	{ label: "Contact", href: "#contact", onClick: () => scrollToId("contact") },
	];
	React.useEffect(() => {
	// Efeito que roda toda vez que 'open' muda — usado para bloquear o scroll no mobile
		if (open) {
	...
	// O efeito depende apenas do estado 'open'.

	return (
	// Retorno do JSX do componente Header.
		<header
			className={cn(
			// BASE: header fixo, centralizado, com largura máxima e transições no desktop
			'fixed top-0 left-1/2 -translate-x-1/2 z-99 w-full max-w-5xl border-b border-transparent md:rounded-md md:border md:transition-all md:ease-in',

			{
				// Quando DER SCROLL e o menu NÃO estiver aberto:
				// aplica fundo semi-transparente, blur, borda, sobe um pouco e reduz a largura no desktop
				'bg-[#00001A95] supports-[backdrop-filter]:bg-[#00001A50] border-[#C88D76] backdrop-blur-lg md:top-6 md:max-w-4xl md:shadow':
				scrolled && !open,

				// Quando o menu estiver ABERTO: fundo sólido, sem reduzir largura
				'bg-[#00001A95]':
				open,
			}
			)}
		...
		>
			...
			>
				...
				{/* Exibe a logo SVG. A classe "h-4" define a altura do ícone. */}

				<div className="hidden items-center gap-2 md:flex">
					{/* Container dos links → esconde no mobile (hidden) e mostra no desktop (md:flex). */}
					...
					{links.map((link) => (
						<a
							...
							link.onClick?.();       // ✅ usa ScrollSmoother
							}}
						>
							{link.label}
						</a>
			...
			<div
				className={cn(
					// Estilização base do menu mobile: ocupa a tela, tem fundo escuro translúcido.

					'bg-neutral-700/90 fixed top-14 right-0 bottom-0 left-0 z-50 flex flex-col overflow-hidden border-y md:hidden',
					open ? 'block' : 'hidden',
					// Se "open" é true → aparece.
            		// Se não → fica oculto.
				)}
			>
				<div
...
export const WordmarkIcon = (props: React.ComponentProps<"svg">) => (
	// Define o componente "WordmarkIcon", que recebe props como className ou style.

  <svg viewBox="0 0 84 24" fill="currentColor" {...props}>
	{/* viewBox define a área de desenho do SVG.
    fill="currentColor" → pega a cor via CSS (ex.: text-white). */}
	...
	{/* Caminho vetorial que forma a logo. É apenas desenho. */}
  </svg>
);